...
import { ConsultantService } from '@portal/core/graphql';
import {EPhase} from '@portal/core/constants';
import { OrdersAmountService } from 'projects/core/src/lib/graphql/orders-amount.service';
import { IConsultantRequest } from '../../../../models/request/consultant-request.interface';
import {capitalizeWords, createPossible} from '@portal/core/utils';
import { IConsultantRetentionUI } from '../../../../models/ui/consultantRetention-ui.interface';
...
export class RetentionListComponent implements OnInit {
  ...
  totalRecords: number;
  pageLinkSize: number;
  filteredConsultants: IConsultantRetentionUI[];
  segmentsData = [];
  filterForm: FormGroup;
  currentPage: number;
  totalPages: number;
  private consultantData: IConsultantRetentionUI[];
    ...
    this.consultantService.getConsultants(request).subscribe(
      res => {
        if (res.Consultants.length > 0) {
          this.consultantData = res.Consultants.map<IConsultantRetentionUI>(x => {
            return {
              id: x.id,
              ...
              constancy: x.constancy.toUpperCase(),
              orderAmount: 0,
  ...
  private getSegments(data: IConsultantRetentionUI[]) {

    let filtered = data;

    if (!this.businessService.isSale) {
      filtered = data.filter(x => !x.has_billed_order);
    }

    const distinct = Array.from(new Set(filtered.map(x => x.constancy)))
      .map(item => {
        return {
          id: item,
          name: createPossible(item),
          count: data.filter(x => x.constancy === item).length
        };
      });

    this.segmentsData = distinct.filter(x => x.id !== '6d6').sort((a, b) => {
          if (a.name > b.name) {
            return 1;
          }
          if (a.name < b.name) {
  ...
  private getOrdersAmount() {
    ...
    this.ordersAmountService.getOrdersAmount(request).subscribe(res => {
      ...
      this.consultantData.forEach((item: IConsultantRetentionUI) => {
  ...
  private search(): IConsultantRetentionUI[] {
    const nameSearch = (this.filterForm.value.nameSearch).toUpperCase();
    const selectedSegmentIds = this.filterForm.controls.segments.value
      .map((v, i) => v ? this.segmentsData[i].id : null)
      .filter(v => v !== null);
    let filtered = this.consultantData;
    if (selectedSegmentIds.length > 0) {
      if (this.businessService.isSale) {
        filtered = filtered.filter(x => selectedSegmentIds.includes(x.constancy));
      } else {
        filtered = filtered.filter(x => selectedSegmentIds.includes(x.constancy) && !x.has_billed_order);
      }
    }
    if (nameSearch) {
